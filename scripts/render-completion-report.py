...
CANONICAL_SECURITY_STATEMENT: Mapping[str, Any] = {
    ...
    "key_rotation_behavior": "The worker performs scheduled gateway signing-key rotation. New keys become active while old public keys remain in JWKS long enough for issued access tokens and verifier caches to expire; expired historical keys are retired automatically. Version 1 exposes no Admin API or CLI operation to force signing-key rotation, select a new active key, or shorten JWKS overlap. Scheduled rotation is not emergency containment.",
