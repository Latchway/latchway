...
def valid_version(value: Any) -> bool:
    """SDKs release independently; each coordinate is still one exact version."""
...
def required_documents(sdk: str, version: str) -> dict[str, str]:
    return {**SDK_SPECS[sdk]["required_documents"], **ADDITIONAL_DOCUMENTS.get((sdk, version), {})}
...
def validate_bundle(sdk: str, version: str, raw: bytes, repository_path: Path | None = None) -> dict[str, Any]:
    if sdk not in SDK_SPECS or not valid_version(version):
        raise BundleError(f"unsupported SDK documentation coordinate: {sdk}@{version}")
    spec = SDK_SPECS[sdk]
    members, epoch = archive_members(raw, version)
    documents = required_documents(sdk, version)
    required = {**COMMON_KINDS, **documents}
    ...
    for index, record_value in enumerate(records):
        ...
        if path in documents or path == "release-notes.md":
            if len(validated) != 1:
...
def lock_entry(sdk: str, result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "archive": f"docs/sdk-bundles/{sdk}/docs-bundle-{manifest['release']['version']}.tar.gz",
...
def read_lock(require_complete: bool) -> dict[str, Any]:
    ...
    for index, entry_value in enumerate(entries):
        ...
        if (
            not valid_version(entry["version"])
            or entry["release"] != f"v{entry['version']}"
            or COMMIT.fullmatch(str(entry["commit"])) is None
            or entry["archive"] != f"docs/sdk-bundles/{sdk}/docs-bundle-{entry['version']}.tar.gz"
...
def render_mdx_snippet(
    ...
    if entry["source_tree_clean"]:
        source_note = (
            ...
            f"Bundle SHA-256: `{entry['archive_sha256']}`."
        )
    else:
        source_note = (
...
def render_inline_code(value: Any, label: str) -> str:
    ...
    # npm peer alternatives contain ||. Escape pipes inside table cells rather
...
def render_sdk_page(sdk: str, entry: Mapping[str, Any], result: Mapping[str, Any]) -> bytes:
    ...
    lines = [
        ...
        f'description: "Generated, version-bound examples and compatibility metadata for {title} {entry["version"]}."',
        "---", "", "<Warning>",
        ...
        f"| Documented SDK release | `{manifest['release']['tag']}` |",
        f"| Documentation source commit | `{manifest['release']['commit']}` |",
    ...
    for path in sorted(required_documents(sdk, entry["version"])):
...
def render_outputs(locked: list[tuple[dict[str, Any], dict[str, Any]]]) -> dict[Path, bytes]:
    ...
    overview = [
        ...
        "</Warning>", "", "| SDK | Package | Documented release | Source tree clean | Documentation commit | Bundle SHA-256 |",
        "| --- | --- | --- | --- | --- | --- |", *overview_rows, "", "## Update a bundle", "",
        "Run the source repository's bundle producer, then import the exact coordinate:", "",
        "```shell", *[f"scripts/docs-sync-sdk {entry['id']} {entry['version']}" for entry, _ in locked], "```", "",
...
def sync_all() -> None:
    ...
    for sdk in sorted(SDK_SPECS):
        ...
        config = load_json(regular_source(repository, "docs-bundle.config.json").read_bytes(), f"{sdk} bundle configuration")
        version = config.get("version") if isinstance(config, dict) else None
        if not valid_version(version) or config.get("release") != f"v{version}":
            raise BundleError(f"{sdk} bundle configuration must declare one exact release")
