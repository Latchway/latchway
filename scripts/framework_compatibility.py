...
def render_markdown(registry: Mapping[str, Any]) -> str:
    ...
    component_rows = [
        {
            ...
            "support": _state(item["support"]),
            "tested": (
                f"{item['tested']['minimum']}–{item['tested']['latest']}"
                if item["tested"]["minimum"] is not None
                else "Not yet tested"
            ),
            "minimum": item["tested"]["minimum"] or "",
            "latest": item["tested"]["latest"] or "",
            "dpop": _state(item["security"]["dpop"]),
            "streaming": item["capabilities"]["streaming"] is True,
            "tools": _state(item["capabilities"]["tools"]),
            "structured": _state(item["capabilities"]["structured_output"]),
            "appExtensions": _state(item["capabilities"]["app_extensions"]),
        }
    ...
    lines = [
        ...
        + json.dumps(component_rows, ensure_ascii=False, separators=(",", ":"))
        + ";",
        "",
