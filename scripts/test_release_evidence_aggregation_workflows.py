...
class ReleaseEvidenceAggregationWorkflowTests(unittest.TestCase):
    def test_cloud_finalizer_authenticates_all_five_platform_producers(self) -> None:
        ...
        for required in (
            ...
            'resolved_digest not in {expected_index_digest, expected_amd64_digest}',
            'def require_database_pool(value, expected, platform):',
            '(5, 3, 2) if platform == "cloudflare_containers" else (20, 15, 5)',
            'set(control) != {"platform", "project", "gateway", "image", "database_pool"}',
            'set(control) != {"platform", "service", "task_definition", "tasks", "database_pool"}',
            'set(control) != {"platform", "app", "machines", "database_pool"}',
            'set(control) != {"platform", "worker", "container", "database_pool"}',
