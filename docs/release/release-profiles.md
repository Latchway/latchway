...
- Docker Compose, Google Cloud Run, AWS, Fly.io, Cloudflare Containers, and
  every other cloud-deployment evidence claim;
...
supply-chain runs and attempts; requires the core record to contain no
deployment evidence and to retain the complete cloud-deployment domain as
deferred; checks out the six exact public repository commits to prove source,
promotion preconditions, and annotated release tags; preserves the canonical
strict report with its expected failed all-domain evidence-window gate;
derives a separately named profile-local input by changing only the
`local_promotion` domain projection; and gives a fresh no-checkout signer the
final decision. That signer independently
...
domain must pass. Retain the exact source report, the three required external
...
The workflow intentionally has no Compose, Google Cloud Run, AWS, Fly.io,
Cloudflare Containers, Mintlify, device, provider, resilience, or reviewer
inputs. It fails if main
...
profile-local input, the sealed authority manifest, all three selected external
...
Before its first mutation it authenticates that producer run, verifies its
GitHub attestation, revalidates the closed candidate artifacts, reruns local
release-tooling and Go gates, and verifies the candidate
...
or overwrites a divergent coordinate. Its core record carries
`deployment_evidence: {}` and explicitly lists `cloud_deployments` as deferred;
it cannot claim any Compose or cloud target was exercised.
Repository release immutability has already been verified live for this launch,
so this selected workflow does not use an administration environment or an
administration token. It still requires the published release to report
`immutable: true`, preserves the exact asset closure, performs the ETag/304
pre-mutation check, and repeats all postpublication tag, release, asset, and
...
The selected core publisher has no deployment-evidence producer, cloud
environment, administration environment, or custom administration secret. It
uses only the run-scoped GitHub token with job-specific permissions and the
two publishing sentinels above. The separate deployment environments and
their cloud credentials remain available only for later deferred evidence or
the unchanged strict profile.
