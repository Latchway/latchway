...
- Apple App Attest uses the 32 digest bytes as `clientDataHash` for attestation or assertion generation and validates the corresponding nonce/signature server-side. Assertion verification computes Apple's nonce as SHA-256 of `authenticatorData || clientDataHash`, then verifies ECDSA-SHA256 over that nonce; digest-oriented cryptography APIs therefore receive SHA-256 of the nonce. Latchway accepts only Apple's legacy compact assertion flags and the current iOS 27 compact forms that set the attested-data and/or extension-data markers while placing the extension map directly after the 37-byte assertion header. When Apple supplies the paired CodeDirectory-hash extensions, Latchway requires a nonzero 32-byte SHA-256 digest and its exact type marker. The authenticated extension bytes remain covered by the assertion signature and stored evidence hash; they are not an independent operator policy input.
