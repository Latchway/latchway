...
| 8 | Quota, pricing and usage settlement | Hard UTC-calendar request-count, UTC-calendar/per-request output-token rules, and durable request/stream concurrency leases have atomic reserve/execute/settle, exact applied-cap reservations, provider-usage release, conservative unknown charging, provenance-bearing usage, entryless non-applicable-stream/per-request lifecycle, contention, bounded stateful recovery, and authenticated PostgreSQL proof. Concurrency retains lease audit rows, counts only active occupancy, releases on every terminal path, and never emits usage. Immutable USD catalogs add exact separately rounded integer nano-USD attribution, exact activation boundaries, catalog/revision provenance, explicit-zero records, and unknown-cost preservation without trusting provider price extensions; full engine remains | Token-bucket, hard-cost, snapshot, override, upstream-reported-pricing, and retry-cost gates remain open |
...
1. Add token buckets, then quota snapshots; keep hard cost limits capability-gated.
