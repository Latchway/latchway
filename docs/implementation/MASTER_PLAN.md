...
| 8 | Quota, pricing and usage settlement | Hard UTC-calendar request-count, bounded rolling `logical_requests` token buckets, UTC-calendar/per-request output-token rules, and durable request/stream concurrency leases have atomic reserve/execute/settle, exact applied-cap reservations, provider-usage release, conservative unknown charging, provenance-bearing usage, entryless non-applicable-stream/per-request lifecycle, contention, bounded stateful recovery, and authenticated PostgreSQL proof. Request token buckets use exact integer refill and lifecycle-safe debit/refund semantics without calendar counters. Concurrency retains lease audit rows, counts only active occupancy, releases on every terminal path, and never emits an occupancy usage row. Immutable USD catalogs add exact separately rounded integer nano-USD attribution, exact activation boundaries, catalog/revision provenance, explicit-zero records, and unknown-cost preservation without trusting provider price extensions; full engine remains | `output_tokens/token_bucket`, hard-cost, snapshot, override, upstream-reported-pricing, and retry-cost gates remain open; general token-bucket support is not claimed |
...
The bounded request-token milestone passes at core `971e7f63a3ed2cc619ef93fd92b296caa216b6cd` from quota-store `c57aed5`, configuration `89f4489`, and dataplane/E2E `971e7f6`. One semantic request is exactly `1,000,000,000,000` balance quanta in a `rolling` bucket. Refill credits only complete `1us` PostgreSQL ticks; capacity is bounded to 1 through `9,223,372`, and rates from `0.000001` through `1,000,000` requests/second must be exactly representable with at most six decimal places. Bucket counters remain `used_units=0` and `reserved_units=0`, while each accepted reservation entry carries semantic unit 1. Denials have positive retry guidance and token-only decisions have zero calendar reset. Accepted replay remains stable and denied replay remains mutation-free; pre-dispatch release and undispatched expiry refund, while a begun attempt, settlement, or dispatched expiry keeps the debit. Refund saturation discards stale full-bucket refill credit, and policy transitions use the lower old/new capacity and rate for the unprocessed interval before rebasing, preventing capacity-increase gifts. Full PostgreSQL normal/race suites, full vet and contract validation, both changed configuration fuzz targets, token-store and authenticated E2E count-3 repetitions, deterministic unchanged bundle SHA-256 `74fc7ada8d835d46b25f763a703b79003cdc8243d6f4b2509645e5a82367ab12`, zero `api`/migration diff, and clean P0-P2 reviews pass.

## Immediate execution sequence

1. Add `output_tokens` token buckets, then quota snapshots; keep hard cost limits capability-gated.
2. Broaden routing and protocol adapters after the remaining quota primitives are durable.
