...
Add concurrency leases, then token buckets. Keep unsupported configuration shapes, including hard cost limits, from activating until their runtime capability exists.
...
| --- | --- | --- |
| `latchway` | `58e8373bdb552496e6bb4c420832e5bb341dd4b3` | Full PostgreSQL-enabled normal and race Go suites; full vet and contract validation; four relevant fuzz smokes; PostgreSQL pricing E2E repeated three times and under race; two deterministic bundle builds at the unchanged hash; zero `api` and migration diff |
...
- Hard calendar request-count and calendar/per-request output-token rules are executable, and configured integer nano-USD attribution is durable. Concurrency leases, token buckets, hard cost limits, input/total-token limits, quota snapshots, user overrides, upstream-reported pricing, and retry/cost accounting remain open.
...
Implement concurrency leases, then token buckets. Hard cost limits remain capability-gated.
