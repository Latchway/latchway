...
| --- | --- | --- | --- | --- |
| Server and CLI | `github.com/latchway/latchway` | Latest local authenticated mixed-quota/configured-pricing/concurrency/token-bucket/hard-calendar-cost proxy and read-only quota-snapshot evidence for supported context-stable plans, plus full PostgreSQL-enabled normal/race, vet, contracts, configuration/policy fuzzing, focused hard-cost PostgreSQL/authenticated E2E normal/race runs, reproducible unchanged bundle, zero `api`/migration/generated-database diff, and clean P0-P2 review at `6304b655d0e2690cbe154b4a66c4ec87966f4387`; earlier `make check` and local non-root OCI evidence at `0a03d9369c0ebcf793f00bac6b002d1caaea6b8e` | Source of truth | Remaining quota work, user overrides, input/total-token limits, other/input-priced cost shapes, upstream-reported pricing/retries, native attestation, live canary/SDK conformance, operations, current Compose/registry image proof, and release gates |
