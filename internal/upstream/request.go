...
func PrepareRequest(
	...
	return prepareRequestAtURL(incoming, target, resolved, forwardedHeaders, staticHeaders)
}
...
func prepareRequestAtURL(
	...
	prepared := PreparedRequest{target: target, request: outbound, state: &preparedRequestState{}}
...
func (target *Target) validatePreparedRequest(prepared PreparedRequest) error {
	...
	for name, values := range request.Header {
		...
		for _, value := range values {
			...
			if !validHeaderValue(value) || totalHeaderBytes > 64<<10 {
