...
func newRoutesSimulateCommand(opts *options, root *controlCommandOptions) *cobra.Command {
	...
	var requestedInput, requestedOutput int64
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if requestedInput < 0 || requestedOutput < 0 || requestedInput > 100_000_000 || requestedOutput > 100_000_000 || len(appVersion) > 128 || strings.ContainsAny(appVersion, "\r\n\x00") {
	...
	command.Flags().Int64Var(&requestedOutput, "requested-output-max", 0, "requested output-token maximum (reported as currently non-decisional)")
...
func printRouteSimulation(opts *options, result routeSimulationResultCLI) error {
	...
	if err := printControlTable(opts, []string{"ALLOWED", "FEATURE", "PROTOCOL", "LIMIT PLAN", "ROUTE", "UPSTREAM", "MODEL", "PHYSICAL MODEL", "PRICING"}, [][]string{{
		boolLabel(result.Allowed), result.Feature, result.Protocol, result.LimitPlan, result.Route,
