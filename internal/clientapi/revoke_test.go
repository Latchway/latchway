...
func TestRevokeCurrentInstallationRejectsAdversarialTransportBeforeCoordinator(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "invalid protocol", mutate: func(r *http.Request) { r.Header.Set("X-Latchway-Protocol-Version", "4") }, wantCode: "protocol_version_unsupported", wantStatus: http.StatusUpgradeRequired},
