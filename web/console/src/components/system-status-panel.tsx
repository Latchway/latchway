...
export function SystemStatusPanel() {
  ...
  return <section className="detail-card" aria-labelledby="system-status-heading">
    ...
    {data ? <dl className="system-status-grid">
      ...
      <div><dt>Database schema</dt><dd>{data.database_schema_version}</dd></div><div><dt>Admin mutations ready</dt><dd>{data.mutation_ready ? "Yes" : "No"}</dd></div>
      <div><dt>Traffic ready</dt><dd>{data.ready ? "Yes" : "No"}</dd></div>
