...
export function SetupWizardPage() {
  ...
  const [workspace, setWorkspace] = useState<{ applicationID: string; applicationSlug: string; clientSurface: "native" | "react_native"; cloudProjectNumber: string; environmentID: string; environmentSlug: string; upstreamAuthentication: "bearer" | "none"; plannedSecretName: string; selfTestMaximumCostNanoUsd: number }>();
  ...
  return <div className="wizard-page">
    ...
    <ol className="wizard-progress" aria-label="Setup progress">{["Create owner", "Create organization", "Create application", "Create environment", "Identity provider", "Attestation & components", "Upstream credential", "Upstream target", "Feature and route", "Limits", "Self-test", "SDK snippets"].map((label, index) => <li className={completed[index] ? "wizard-progress__done" : ""} key={label}><span>{completed[index] ? "✓" : index + 1}</span>{label}</li>)}</ol>
    ...
    {!workspace ? <section className="wizard-card"><h2>Application, environment, identity, and native proof</h2><p>The owner and organization were created by secure bootstrap. Define the first production application and the identifiers the verifiers must pin.</p>
      <form className="control-form" onSubmit={(event) => void createWorkspace(event)}>
        ...
        <div className="form-field-grid"><label>Environment name<input defaultValue="Production" name="environment_name" required /></label><label>Environment slug<input defaultValue="production" name="environment_slug" pattern="[a-z][a-z0-9-]{1,62}" required /></label><label>Firebase project ID<input name="firebase_project" pattern="[a-z][a-z0-9-]{4,28}[a-z0-9]" required /></label><label>Mobile SDK surface<select defaultValue="react_native" name="client_surface" required><option value="react_native">React Native iOS + Android</option><option value="native">Native Swift + Kotlin</option></select><small>The generated Component Definitions bind the exact runtime platforms; create a separate application environment for a second surface.</small></label></div>
