...
export function AdministratorsPage() {
  ...
  const canInspect = session.data.session?.capabilities.includes("manage_owners") ?? false;
  ...
  async function create(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const form = event.currentTarget; const data = new FormData(form); const passwordField = form.elements.namedItem("password");
    const body = {
    ...
    setBusy(true); setProblem(undefined);
    try {
      const item = (await adminRequest("/admin/v1/administrators", AdministratorSchema, { method: "POST", body })).data;
    ...
    finally { if (passwordField instanceof HTMLInputElement) passwordField.value = ""; setBusy(false); }
  }
  ...
  async function resetPassword(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault(); if (!canManage || !resetTarget) return;
    const form = event.currentTarget; const data = new FormData(form); const passwordField = form.elements.namedItem("password");
    ...
    setBusy(true); setProblem(undefined);
    try {
      const updated = (await adminRequest(`/admin/v1/administrators/${resetTarget.id}/reset-password`, AdministratorSchema, { method: "POST", body: { password } })).data;
      setPage((current) => current ? { ...current, items: current.items.map((entry) => entry.id === updated.id ? updated : entry) } : current);
  ...
  return <div className="control-page">
    ...
    {!canInspect ? <div className="control-notice"><strong>Owner access required</strong><span>Your current role can’t manage administrator accounts.</span></div> : canManage ? <form className="control-form" onSubmit={(event) => void create(event)}>
    ...
    <div className="button-row"><button className="secondary-action" disabled={busy || !canInspect} onClick={() => void load()} type="button">Load administrators</button></div><ProblemNotice problem={problem} />
    ...
    {disableTarget ? <ImmediateOperationConfirmation acknowledgement="I understand this immediately removes organization access and permanently revokes the administrator's current scoped sessions and API tokens." affectedScope={<><code>{disableTarget.id}</code> ({disableTarget.email}) in organization <code>{session.data.session?.organization_id}</code></>} busy={busy} confirmLabel="Disable and revoke credentials" credentialRestoration="Never. Enabling the membership later permits future authentication, but it does not restore revoked sessions or API tokens." disabled={!canManage} heading={`Disable ${disableTarget.email}?`} key={disableTarget.id} onCancel={() => setDisableTarget(undefined)} onConfirm={() => void mutate(disableTarget, "disable", undefined, "POST")} reversibility="Yes. An owner can enable the organization membership later." summary="The membership is disabled and its existing sessions and API tokens in this organization are revoked transactionally. The server protects the final active owner." timing="Immediately after the server accepts the action" /> : null}
    {resetTarget && canManage ? <form className="detail-card" onSubmit={(event) => void resetPassword(event)}><h2>Reset {resetTarget.email}</h2><p>This revokes the administrator’s active sessions and API tokens. Cross-organization accounts require a separate recovery process.</p><label>Replacement password<input autoComplete="new-password" minLength={12} name="password" required type="password" /></label><div className="button-row"><button className="primary-action" disabled={busy || !canManage} type="submit">Reset and revoke credentials</button><button className="secondary-action" disabled={busy} onClick={() => setResetTarget(undefined)} type="button">Cancel</button></div></form> : null}
  </div>;
}
