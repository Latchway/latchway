...
function RequestsWorkspacePage() {
  ...
  return <div className="control-page">
    ...
    <ProblemNotice problem={problem} />
    {page ? <><Table headers={["Time", "Status", "Feature", "User", "Component / trust", "Selected route / model", "Latency", "Tokens", "Cost"]} rows={page.items.map((request) => { const attempt = request.attempts.at(-1); return [time(request.started_at), <button aria-label={request.id} className="link-button" disabled={detailBusy} onClick={() => selectRequest(request.id)} type="button">{request.status}<br /><small>{request.id}</small></button>, request.feature, request.user_id, `${request.component_kind ?? "legacy"} · ${request.trust_source ?? "legacy trust"}`, request.selected_route ? `${request.selected_route} → ${request.selected_model}` : attempt ? `${attempt.route} → ${attempt.model} (observed)` : "Unavailable", duration(request.started_at, request.completed_at), displayUsageMetric(request.usage, "total_tokens"), displayUsageMetric(request.usage, "cost_nano_usd")]; })} />{page.page.has_more && page.page.next_cursor ? <button className="secondary-action" disabled={busy} onClick={() => nextPage(page.page.next_cursor ?? "")} type="button">Next page</button> : null}</> : null}
    ...
    {selected ? <aside className="detail-card request-detail"><div className="detail-card__heading"><h2>Request detail</h2><button className="small-action" onClick={() => { setSelected(undefined); setEffective(undefined); workspace?.updateSearch({ request: undefined }); }} type="button">Close detail</button></div>
      ...
      {effective ? <EffectiveConfigurationPanel configuration={effective} /> : null}
      <h3>Client attribution</h3><dl><div><dt>Caller SDK</dt><dd>{selected.caller_sdk ?? "historical request"}</dd></div><div><dt>Installation</dt><dd>{selected.installation_id}</dd></div><div><dt>Installation Family</dt><dd>{selected.installation_family_id ?? "legacy request"}</dd></div><div><dt>Client component</dt><dd>{selected.client_component_id ?? "legacy request"}</dd></div><div><dt>Component definition</dt><dd>{selected.component_definition_id ?? "legacy request"}</dd></div><div><dt>Component kind</dt><dd>{selected.component_kind ?? "legacy request"}</dd></div><div><dt>Trust source</dt><dd>{selected.trust_source ?? "legacy request"}</dd></div><div><dt>Framework</dt><dd>{selected.framework ? `${selected.framework}${selected.framework_version ? ` ${selected.framework_version}` : ""}` : "raw transport"}</dd></div><div><dt>Framework version</dt><dd>{selected.framework_version ?? "—"}</dd></div></dl>
      <h3>Usage and quota accounting</h3><p>Recorded ledger units are not necessarily generated tokens or a provider bill. Unknown usage can carry conservative quota charges; missing cost is not a zero bill.</p><Table headers={["Logical requests", "Input tokens", "Output tokens", "Total tokens", "Cost nano-USD"]} rows={selected.usage ? [[selected.usage.logical_requests, displayUsageMetric(selected.usage, "input_tokens"), displayUsageMetric(selected.usage, "output_tokens"), displayUsageMetric(selected.usage, "total_tokens"), displayUsageMetric(selected.usage, "cost_nano_usd")]] : []} />
      <h3>Ordered upstream attempts</h3><Table headers={["#", "Attempt", "Route", "Started", "First byte", "First token", "TTFT", "Completed", "Duration", "Upstream", "Model", "Status", "HTTP", "Failure", "Input", "Output", "Total", "Cost nUSD", "Usage provenance", "Cost provenance", "Cost source"]} rows={selected.attempts.map((attempt) => [attempt.attempt_number, attempt.id, attempt.route, time(attempt.started_at), time(attempt.first_byte_at), time(attempt.first_token_at), duration(attempt.started_at, attempt.first_token_at), time(attempt.completed_at), duration(attempt.started_at, attempt.completed_at), attempt.upstream, attempt.model, attempt.status, attempt.http_status ?? "—", attempt.failure_code ? <FailureCodeLink code={attempt.failure_code} /> : "—", displayUsageMetric(attempt.usage, "input_tokens"), displayUsageMetric(attempt.usage, "output_tokens"), displayUsageMetric(attempt.usage, "total_tokens"), displayUsageMetric(attempt.usage, "cost_nano_usd"), attempt.usage_provenance, attempt.cost_provenance, attempt.cost_source ?? "—"])} />
      <h3>Safe provider diagnostics</h3><Table headers={["Attempt", "Accounting policy", "Category", "Parameter", "Provider code", "Generation ID", "Request ID"]} rows={selected.attempts.filter((attempt) => attempt.provider_error || attempt.accounting_policy).map((attempt) => [attempt.attempt_number, attempt.accounting_policy || "Diagnostics only", attempt.provider_error?.category ?? "—", attempt.provider_error?.parameter ?? "—", attempt.provider_error?.provider_code ?? "—", attempt.provider_error?.generation_id ?? "—", attempt.provider_error?.request_id ?? "—"])} />
      <h3>Conservative input-bound breakdown</h3><p>These aggregate sizes explain the reservation bound, not the provider's measured token count. Historical requests may not have this breakdown.</p><Table headers={["Attempt", "Rewritten bytes", "Framing units", "Per-request allowance", "Per-unit allowance", "Expanded schema bytes"]} rows={selected.attempts.flatMap((attempt) => attempt.input_accounting_breakdown ? [[attempt.attempt_number, attempt.input_accounting_breakdown.rewritten_request_bytes, attempt.input_accounting_breakdown.framing_unit_count, attempt.input_accounting_breakdown.maximum_framing_tokens_per_request, attempt.input_accounting_breakdown.maximum_framing_tokens_per_unit, attempt.input_accounting_breakdown.expanded_schema_bytes]] : [])} />
...
function FocusedAnalytics({ focus, series, summary }: { focus: AnalyticsFocus; series?: UsageTimeseries; summary: UsageSummary }) {
  if (focus === "cost") return <>
    <section className="metric-grid"><article><span>Total cost</span><strong>{displayUsageMetric(summary.values, "cost_nano_usd")}</strong></article><article><span>Cost / active user</span><strong>{summary.values.details?.cost_nano_usd.recorded_units === null ? "Not recorded" : `${ratio(summary.analytics.cost_per_active_user_nano_usd, " nUSD")} (ledger)`}</strong></article><article><span>Requests</span><strong>{summary.analytics.request_count.toLocaleString()}</strong></article></section>
    <section className="detail-card"><h2>Cost by feature</h2><Table headers={["Feature", "Requests", "Active users", "Cost nano-USD"]} rows={summary.analytics.by_feature.items.map((item) => [item.key, item.request_count, item.active_users, displayUsageMetric(item.values, "cost_nano_usd")])} /><BreakdownLimitNotice label="feature" limit={summary.analytics.by_feature.limit} truncated={summary.analytics.by_feature.truncated} /></section>
    <section className="detail-card"><h2>Cost by model</h2><Table headers={["Model", "Requests", "Active users", "Cost nano-USD"]} rows={summary.analytics.by_model.items.map((item) => [item.key, item.request_count, item.active_users, displayUsageMetric(item.values, "cost_nano_usd")])} /><BreakdownLimitNotice label="model" limit={summary.analytics.by_model.limit} truncated={summary.analytics.by_model.truncated} /></section>
    <section className="detail-card"><h2>Cost by selected plan</h2><Table headers={["Selected plan", "Requests", "Active users", "Cost nano-USD"]} rows={summary.analytics.by_selected_plan.items.map((item) => [item.key, item.request_count, item.active_users, displayUsageMetric(item.values, "cost_nano_usd")])} /><BreakdownLimitNotice label="selected-plan" limit={summary.analytics.by_selected_plan.limit} truncated={summary.analytics.by_selected_plan.truncated} /></section>
    <section className="detail-card"><h2>Cost provenance</h2><Table headers={["Provenance", "Fixed source", "Cost nano-USD"]} rows={summary.analytics.usage_by_provenance.map((item) => [item.provenance, item.cost_source ?? "—", displayUsageMetric(item.values, "cost_nano_usd")])} /></section>
    {series ? <Table headers={["Bucket", "Cost nano-USD"]} rows={series.points.map((point) => [time(point.timestamp), displayUsageMetric(point.values, "cost_nano_usd")])} /> : null}
  </>;
  ...
  return <>
    <section className="metric-grid"><article><span>Active AI users</span><strong>{summary.analytics.active_users.toLocaleString()}</strong></article><article><span>Requests / active user</span><strong>{ratio(summary.analytics.requests_per_active_user)}</strong></article><article><span>Logical requests</span><strong>{summary.values.logical_requests.toLocaleString()}</strong></article><article><span>Input tokens</span><strong>{displayUsageMetric(summary.values, "input_tokens")}</strong></article><article><span>Output tokens</span><strong>{displayUsageMetric(summary.values, "output_tokens")}</strong></article><article><span>Total tokens</span><strong>{displayUsageMetric(summary.values, "total_tokens")}</strong></article></section>
    <section className="detail-card"><h2>Feature usage</h2><Table headers={["Feature", "Active users", "Requests", "Input", "Output", "Total"]} rows={summary.analytics.by_feature.items.map((item) => [item.key, item.active_users, item.request_count, displayUsageMetric(item.values, "input_tokens"), displayUsageMetric(item.values, "output_tokens"), displayUsageMetric(item.values, "total_tokens")])} /><BreakdownLimitNotice label="feature" limit={summary.analytics.by_feature.limit} truncated={summary.analytics.by_feature.truncated} /></section>
    <section className="detail-card"><h2>Model usage</h2><Table headers={["Model", "Active users", "Requests", "Input", "Output", "Total"]} rows={summary.analytics.by_model.items.map((item) => [item.key, item.active_users, item.request_count, displayUsageMetric(item.values, "input_tokens"), displayUsageMetric(item.values, "output_tokens"), displayUsageMetric(item.values, "total_tokens")])} /><BreakdownLimitNotice label="model" limit={summary.analytics.by_model.limit} truncated={summary.analytics.by_model.truncated} /></section>
    <section className="detail-card"><h2>Selected-plan usage</h2><Table headers={["Selected plan", "Active users", "Requests", "Input", "Output", "Total"]} rows={summary.analytics.by_selected_plan.items.map((item) => [item.key, item.active_users, item.request_count, displayUsageMetric(item.values, "input_tokens"), displayUsageMetric(item.values, "output_tokens"), displayUsageMetric(item.values, "total_tokens")])} /><BreakdownLimitNotice label="selected-plan" limit={summary.analytics.by_selected_plan.limit} truncated={summary.analytics.by_selected_plan.truncated} /></section>
    <section className="detail-card"><h2>Usage provenance</h2><Table headers={["Provenance", "Requests", "Input", "Output", "Total"]} rows={summary.analytics.usage_by_provenance.map((item) => [item.provenance, item.values.logical_requests, displayUsageMetric(item.values, "input_tokens"), displayUsageMetric(item.values, "output_tokens"), displayUsageMetric(item.values, "total_tokens")])} /></section>
    {series ? <Table headers={["Bucket", "Requests", "Input", "Output", "Total"]} rows={series.points.map((point) => [time(point.timestamp), point.values.logical_requests, displayUsageMetric(point.values, "input_tokens"), displayUsageMetric(point.values, "output_tokens"), displayUsageMetric(point.values, "total_tokens")])} /> : null}
  </>;
}
...
export function RouteSimulatorPage() {
  ...
  const [expandedSchemaBytes, setExpandedSchemaBytes] = useState(routeSearch.expanded_schema_bytes ?? "0");
  ...
  useEffect(() => {
    ...
    setExpandedSchemaBytes(routeSearch.expanded_schema_bytes ?? "0");
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      const safeSearchCandidate = {
        ...
        authenticated,
        environment_id: environment || undefined,
        expanded_schema_bytes: expandedSchemaBytes || "0",
        feature,
      ...
      const response = await adminRequest(`/admin/v1/config-revisions/${revision}/simulate`, RouteSimulationSchema, { method: "POST", body: {
        ...
        request: { streaming: streamingSimulation, app_version: appVersion, requested_input_tokens: Number(requestedInputTokens || 0), requested_output_max: Number(requestedOutputMaximum || 0), rewritten_request_bytes: Number(rewrittenRequestBytes || 0), framing_unit_count: Number(framingUnitCount || 0), expanded_schema_bytes: Number(expandedSchemaBytes || 0) }
      }});
      ...
      if (workspace && safeSearch) workspace.updateSearch({
        ...
        expanded_schema_bytes: safeSearch.expanded_schema_bytes,
