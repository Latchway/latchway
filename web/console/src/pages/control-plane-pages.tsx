...
export function UsagePage() {
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      const [summaryResult, seriesResult] = await Promise.all([
        adminRequest(queryPath("/admin/v1/usage/summary", query), UsageSummarySchema),
  ...
  return <div className="control-page">
    ...
    {summary ? <section className="metric-grid"><article><span>Logical requests</span><strong>{summary.values.logical_requests.toLocaleString()}</strong></article><article><span>Total tokens</span><strong>{summary.values.total_tokens.toLocaleString()}</strong></article><article><span>Cost, nano-USD</span><strong>{summary.values.cost_nano_usd.toLocaleString()}</strong></article><article><span>Provenance</span><strong>{summary.provenance.join(", ") || "none"}</strong></article></section> : null}
...
export function RouteSimulatorPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      const response = await adminRequest(`/admin/v1/config-revisions/${revision}/simulate`, RouteSimulationSchema, { method: "POST", body: {
        ...
        request: { streaming: form.get("streaming") === "on", app_version: String(form.get("app_version") ?? ""), requested_input_tokens: Number(form.get("requested_input") ?? 0), requested_output_max: Number(form.get("requested_output") ?? 0) }
      }});
  ...
  return <div className="control-page"><PageHeading eyebrow="Operations" title="Route simulator">The server executes the exact compiled production CEL and resolver. This performs no quota reservation and no upstream dispatch.</PageHeading>
    <form className="control-form" onSubmit={(event) => void submit(event)}>
      ...
      <div className="form-field-grid"><label>App version<input maxLength={128} name="app_version" /></label><label>Requested input tokens<input min={0} name="requested_input" type="number" /></label><label>Requested output maximum<input min={0} name="requested_output" type="number" /></label></div>
    ...
    {result ? <section className={`simulation-result ${result.allowed ? "simulation-result--allowed" : "simulation-result--denied"}`}><h2>{result.allowed ? "Allowed" : "Denied"}</h2><dl><div><dt>Access expression</dt><dd>{result.matched_access_expression ?? "—"}</dd></div><div><dt>Limit plan</dt><dd>{result.limit_plan ?? "—"}</dd></div><div><dt>Route</dt><dd>{result.route ?? "—"}</dd></div><div><dt>Upstream</dt><dd>{result.upstream ?? "—"}</dd></div><div><dt>Physical model</dt><dd>{result.physical_model ?? "—"}</dd></div><div><dt>Pricing</dt><dd>{result.pricing_confidence ?? "—"}</dd></div></dl><h3>Explanation</h3><ul>{result.explanation.map((line) => <li key={line}>{line}</li>)}</ul>{result.warnings?.length ? <><h3>Warnings</h3><ul>{result.warnings.map((line) => <li key={line}>{line}</li>)}</ul></> : null}{result.fallback_sequence?.length ? <Table headers={["Route", "Upstream", "Model", "Physical model", "Fallback on"]} rows={result.fallback_sequence.map((candidate) => [candidate.route, candidate.upstream, candidate.model, candidate.physical_model, candidate.fallback_on.join(", ")])} /> : null}</section> : null}
  </div>;
}
