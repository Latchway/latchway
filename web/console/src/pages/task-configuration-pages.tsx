...
function ConnectionWorkspace() {
  ...
  async function stageConnection(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      if (staged) {
        ...
        setLastConnection({ id: connectionID, model: modelID, selfTestSupported: providerType === "openai_compatible" && ["openai_responses", "openai_chat"].includes(protocol) });
      }
    } catch (error) {
  ...
  return <div className="control-page task-workspace">
    ...
    {document ? <>
      ...
      {lastConnection && connectionPublished ? <section className="detail-card"><h2>Test the published connection</h2><p>The bounded self-test uses the selected upstream and model with a 10,000,000 nano-USD ceiling. It does not impersonate an application user.</p>{lastConnection.selfTestSupported ? <button className="primary-action" onClick={() => void runSelfTest()} type="button">Run bounded connection test</button> : <p className="control-notice"><strong>Self-test unavailable for this protocol</strong><span>The Admin API currently supports connection self-tests only for OpenAI-compatible Responses and Chat models. Verify another protocol through a real client request.</span></p>}{selfTest ? <p className="resource-result">Self-test <code>{selfTest.id}</code>: <strong>{selfTest.state}</strong></p> : null}</section> : null}
    </> : null}
  </div>;
}
